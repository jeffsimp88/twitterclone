...
class CustomUserAdmin(UserAdmin):
    ...
    fieldsets = (
        (None, {"fields": ('username', 'display_name', 'email', 'following_user', 'password')}),
            ('Permissions', {'fields': ('is_staff', 'is_active')})
    )
    add_fieldsets  = (
        (None, {
            "classes": ('wide',),
            "fields": ('username', 'display_name', 'email', 'following_user', 'password1', 'password2', 'is_staff', 'is_active')
        }),
    )
